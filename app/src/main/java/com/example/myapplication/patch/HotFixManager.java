package com.example.myapplication.patch;
...
public class HotFixManager {
    ...
    public static void preformHotFix(@NonNull Application application) {
        if (!hasDex(application)) {
            System.out.println("hasDex: " + "false");
...
//            Object[] dexElements = new Object[sizeOfNewDexElement + sizeOfOldDexElement];
    ...
    public static String getDexDir(@NonNull Application application) {
        System.out.println(application.getCacheDir() + File.separator + DEX_DIR);
    ...
    public static void printClassLoaderInfo(Application application) {
        ClassLoader classLoader = application.getClassLoader();
    ...
    public static void printDexPathListInfo(Application application) {
        ClassLoader classLoader = application.getClassLoader();
        try {
            Class<?> clsBaseDexClassLoader = Class.forName("dalvik.system.BaseDexClassLoader");
            Field pathListField = clsBaseDexClassLoader.getDeclaredField("pathList");
    ...
    public static void log(String message) {
        Log.d("xianyu", message);
    }

    public static Handler mMainHandle = new Handler(Looper.getMainLooper());
    public static void downloadHotFixDexFile(Application application) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String dexDir = HotFixManager.getDexDir(application);
                File dirFile = new File(dexDir);
                if (!dirFile.exists()) {
                    dirFile.mkdir();
                }
                File destFile = new File(dexDir, "classes.dex");
                try (InputStream inputStream = application.getAssets().open("dex/classes.dex");
